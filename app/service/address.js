...
class AddressService extends Service {
  async getAddressSummary(addressIds, p2pkhAddressIds, rawAddresses) {
    const {Address} = this.ctx.app.fantasygoldinfo.lib
    ...
    let [
      ...
      fgc20Service.getAllFGC20Balances(hexAddresses),
      fgc721Service.getAllFGC721Balances(hexAddresses),
  ...
  async getAddressTransactionCount(addressIds, rawAddresses) {
    const {Address: RawAddress, Solidity} = this.app.fantasygoldinfo.lib
  ...
  async getAddressFGC20TokenTransactionCount(rawAddresses, token) {
    const {Address, Solidity} = this.app.fantasygoldinfo.lib
    const TransferABI = Solidity.fgc20ABIs.find(abi => abi.name === 'Transfer')
  ...
  async getAddressFGC20TokenTransactions(rawAddresses, token) {
    const {Address, Solidity} = this.app.fantasygoldinfo.lib
    const TransferABI = Solidity.fgc20ABIs.find(abi => abi.name === 'Transfer')
    const db = this.ctx.model
    const {sql} = this.ctx.helper
    ...
    let topicAddresses = rawAddresses
      .filter(address => address.type === Address.PAY_TO_PUBLIC_KEY_HASH)
      .map(address => Buffer.concat([Buffer.alloc(12), address.data]))
    let totalCount = await this.getAddressFGC20TokenTransactionCount(rawAddresses, token)
    let transactions = await db.query(sql`
      SELECT
      ...
      INNER JOIN fgc20 ON fgc20.contract_address = log.address
