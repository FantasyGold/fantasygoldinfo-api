...
class ContractService extends Service {
  ...
  async getContractSummary(contractAddress, addressIds) {
    const {Contract, Fgc20: FGC20, Fgc20Statistics: FGC20Statistics, Qrc721: FGC721} = this.ctx.model
    const {balance: balanceService, fgc20: fgc20Service, fgc721: fgc721Service} = this.ctx.service
    ...
    let [
      ...
      fgc20Service.getAllFGC20Balances([contractAddress]),
      fgc721Service.getAllFGC721Balances([contractAddress]),
    ...
    return {
      ...
      ...contract.type === 'fgc20' ? {
        fgc20: {
          ...
          totalSupply: contract.fgc20.totalSupply,
          version: contract.fgc20.version,
          holders: contract.fgc20.statistics.holders,
          transactions: contract.fgc20.statistics.transactions
        }
      } : {},
      ...contract.type === 'fgc721' ? {
        fgc721: {
          ...
          totalSupply: contract.fgc721.totalSupply
        }
      } : {},
    ...
    let [{count}] = await db.query(sql`
      SELECT COUNT(*) AS count FROM (
        ...
        WHERE log.receipt_id = receipt._id
          AND ${this.ctx.service.block.getRawBlockFilter('receipt.block_height')}
          AND contract.address = log.address AND contract.type IN ('fgc20', 'fgc721')
          AND log.topic1 = ${TransferABI.id}
          AND (log.topic2 = ${topic} OR log.topic3 = ${topic})
          AND (
            (contract.type = 'fgc20' AND log.topic3 IS NOT NULL AND log.topic4 IS NULL)
            OR (contract.type = 'fgc721' AND log.topic4 IS NOT NULL)
          )
      ) list
  ...
  async getContractBasicTransactions(contractAddress) {
    const {Address, OutputScript} = this.app.fantasygoldinfo.lib
    const {
