...
class MiscService extends Service {
  async classify(id) {
    ...
    const {Block, Transaction, Contract, Fgc20: FGC20, where, fn, literal} = db
    ...
    let fgc20Results = (await FGC20.findAll({
      where: {
        [$or]: [
      ...
      transaction
    })).map(fgc20 => fgc20.contractAddress)
    if (fgc20Results.length === 0) {
      fgc20Results = (await FGC20.findAll({
        where: {
          [$or]: [
