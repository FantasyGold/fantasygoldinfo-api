...
class TransactionService extends Service {
  async getTransaction(id) {
    const {
      ...
      Contract, Fgc20: FGC20, Qrc721: FGC721,
      where, col
    } = this.ctx.model
    const {in: $in} = this.app.Sequelize.Op
    const {Address: RawAddress} = this.app.fantasygoldinfo.lib
        ...
        if (output.evmReceipt) {
          ...
          outputObject.evmReceipt.logs = eventLogs.filter(log => log.receiptId === output.evmReceipt._id).map(log => ({
            ...
            ...log.fgc20 ? {
              fgc20: {
                name: log.fgc20.name,
                symbol: log.fgc20.symbol,
                decimals: log.fgc20.decimals
              }
            } : {},
            ...log.fgc721 ? {
              fgc721: {
                name: log.fgc721.name,
                symbol: log.fgc721.symbol
              }
            } : {}
          }))
        }
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.fantasygoldinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.fantasygoldinfo.rpc(this.app.config.fantasygoldinfo.rpc)
  ...
  async transformTransaction(transaction, {brief = false} = {}) {
    ...
    let [fgc20TokenTransfers, fgc721TokenTransfers] = await Promise.all([
      this.transformFGC20Transfers(transaction.outputs),
      this.transformFGC721Transfers(transaction.outputs)
    ])
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.fantasygoldinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.fantasygoldinfo.lib
  ...
  async transformFGC20Transfers(outputs) {
    const TransferABI = this.app.fantasygoldinfo.lib.Solidity.fgc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {address, addressHex, topics, data, fgc20} of output.evmReceipt.logs) {
          if (fgc20 && topics.length === 3 && Buffer.compare(topics[0], TransferABI.id) === 0 && data.length === 32) {
  ...
  async transformQRC721Transfers(outputs) {
    ...
    for (let output of outputs) {
      if (output.evmReceipt) {
        for (let {addressHex, topics, fgc721} of output.evmReceipt.logs) {
          if (fgc721 && topics.length === 4 && Buffer.compare(topics[0], TransferABI.id) === 0) {
            let [from, to] = await this.ctx.service.contract.transformHexAddresses([topics[1].slice(12), topics[2].slice(12)])
            result.push({
              address: addressHex.toString('hex'),
              addressHex: addressHex.toString('hex'),
              name: fgc721.name,
              symbol: fgc721.symbol,
              ...from && typeof from === 'object' ? {from: from.hex.toString('hex'), fromHex: from.hex.toString('hex')} : {from},
              ...to && typeof to === 'object' ? {to: to.hex.toString('hex'), toHex: to.hex.toString('hex')} : {to},
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.fantasygoldinfo.lib
    const {
